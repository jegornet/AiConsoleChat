...
from mcp_client_python import PythonDockerMCPClient
...
async def main():
    ...
    # Создаём MCP клиент
    mcp_client = PythonDockerMCPClient()
    ...
    try:
        ...
        print("\n💡 Возможные причины:")
        print("  - PythonDockerMCPServer сервер не может запуститься")
        print("  - Проблемы с зависимостями Python")
        print("  - Проблемы с доступом к MCP серверу")
        sys.exit(1)

    client = anthropic.Anthropic()
    conversation = []

    system_prompt = """Ты ассистент Python-программиста и у тебя есть доступ к MCP инструменту, выполняющему Python код в Docker контейнере.

Когда пользователь просит выполнить код, используй доступный инструмент execute_python для выполнения
пользовательского кода. 

КРИТИЧЕСКИ ВАЖНО: 
- Выводи ТОЛЬКО чистый результат выполнения кода, без объяснений, комментариев или дополнительного текста
- НЕ анализируй результат  
- НЕ объясняй что означает результат
- НЕ добавляй контекст или интерпретацию
- Просто покажи то, что вернул код - и всё

Если результат вывода пуст, выдай только: "код вернул пустой результат" """
